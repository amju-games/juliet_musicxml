...
TEST_CASE("Parse simple xml file example", "xml_parser")
{
  ...
  const auto& bars = it->m_bars;
  REQUIRE(!bars.empty());
  const auto& bar = bars[0];
  REQUIRE(bar->m_events.size() == 4); // number of events in the bar in the example XML
  // First event should be Music XML divisions
  REQUIRE(dynamic_cast<divisions*>(bar->m_events[0].get())); 
  const auto* divs = dynamic_cast<divisions*>(bar->m_events[0].get());
  ...
  const auto* n = dynamic_cast<note*>(bar->m_events[3].get());

  // Check part 
  REQUIRE(bar->m_part_index == 0); 
  // TODO check part name should match what's in the example XML
...
<note><step>D</step><duration>6</duration></note>
  ...
  REQUIRE(dynamic_cast<note*>(bar->m_events[0].get())); 
  REQUIRE(dynamic_cast<rest*>(bar->m_events[1].get())); 
  REQUIRE(dynamic_cast<rest*>(bar->m_events[2].get()));
